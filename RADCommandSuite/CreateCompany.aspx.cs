...
public partial class CreateCompany : System.Web.UI.Page
{
    ...
    //{
    //    if (grdRADCompany.Columns.Count > 0)
    //    {
    //        grdRADCompany.Columns.Clear();
    //    }

    //    BoundField boundIDField = new BoundField();
    //    boundIDField.HeaderText = "RAD Device ID";
    //    boundIDField.DataField = "RADDeviceID";
    //    this.grdRADCompany.Columns.Add(boundIDField);

    //    BoundField boundByField = new BoundField();
    //    boundByField.HeaderText = "RAD Device Name";
    //    boundByField.DataField = "DeviceInfo";
    //    this.grdRADCompany.Columns.Add(boundByField);

    //    DataTable dtCompanies = this.GetCompanies();
    ...
    //            customField.HeaderTemplate = new MyGridViewTemplate(DataControlRowType.Header, drRow["CompanyName"].ToString());
    //            customField.ItemTemplate = new MyGridViewTemplate(DataControlRowType.DataRow, drRow["CompanyName"].ToString());
    //            grdRADCompany.Columns.Add(customField);
    //        }
    //    }
    //}
    ...
    protected void btnCreateCompany_Click(object sender, EventArgs e)
    {
        ...
        {
            BusinessLogic objBusinessLogic = new BusinessLogic();
            blnResult = objBusinessLogic.CreateCompany(txtCreateCompany.Text);
            if (blnResult)
            {
                lblAddCompanyStatus.Visible = true;
                lblAddCompanyStatus.ForeColor = Color.Green;
                lblAddCompanyStatus.Text = "Company added successfully.";
            }
            else
            {
                lblAddCompanyStatus.Visible = true;
                lblAddCompanyStatus.ForeColor = Color.Red;
                lblAddCompanyStatus.Text = "Operation Failed. Please Contact Administrator.";
            }
        }
        BindGrid();
    }
    ...
    protected void grdCompany_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        BusinessLogic objBussLogic = new BusinessLogic();
        bool blnResult = objBussLogic.UpdateCompany(txtCompanyName.Text, Convert.ToInt32(strCompanyID));
        if (blnResult)
        {
