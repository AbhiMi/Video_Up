...
public partial class Pages_UploadMediaLibrary : System.Web.UI.Page
{
    ...
    protected void btnAssociatePLtoCmpgn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (MediaAssociationList != null && MediaAssociationList.Count > 0)
            {
                ...
                foreach (Values AssociationVal in MediaAssociationList)
                {
                    blnResult = objBusinessLogic.AssociatePlayListToCampaign(AssociationVal.PlayListID, AssociationVal.CampaignID, commFunc.GetUsersCompanyID(Context));
                }
                if (blnResult)
                {
